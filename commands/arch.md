...
Research the **current, dated** state of the art, not only how apps were built in the past: for every fast-moving technology (models, APIs, SDKs, platform frameworks), check **what newer technology now exists** and stamp each finding's `versionOrDate`. Because the market moves fast, prefer a design that keeps such a choice behind a **swappable seam** (a provider protocol) over hardwiring one vendor — and record the alternatives in the decision atom's `rejectedOptions`/`fallback`. This stage decides the stack autonomously; **it never asks the user** to pick a provider, model, or framework.
...
- **Selection follows the contract's stated priority:** when the intent contract states a priority (quality, privacy, cost, latency) via `explicitConstraints`/`qualityAttributes`, `selectionCriteria` must rank by it and `chosenOptionId`/`recommendedOptionId` must follow it — never agent convenience.
...
For **every** MUST requirement, a `coverageMatrix` row linking it to journeys, decisions, components, capabilities, and verifications, with `coverage: "complete"`. Pin the consumed contract in `sourceSpec` (`schemaVersion`, `specRevision`, and a **non-null `contentDigest`** — compute it with `npm run kiln -- digest kiln-spec.json`). Declare `environmentPrerequisites` (build/test/release; `blocking` true/false) — machine prerequisites, **distinct** from `openConfirmations` (unresolved user intent).
...
`status` is `"ready_for_build"` **only** when `openConfirmations` is empty, no decision is `pending_confirmation`, every coverage row is `complete`, and — checked against the consumed contract via `validateArchAgainstIntent` — every `tracesTo`/`capabilityNeedId` resolves to a real intent id, every MUST requirement has a complete coverage row, no `capability` exceeds a `confirmed` capability need, and `sourceSpec` pins the intent by digest. Enforce it: `npm run kiln -- check kiln-spec.json kiln-arch.json` must exit 0 before you declare `ready_for_build` — fix every reported violation first.
